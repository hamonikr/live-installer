...
class Partition(object):
    ...
    def __init__(self, partition):
        ...
        try:
            ...
            description = ''
            if path_exists(mount_point, 'etc/hamonikr/info'):
                description = getoutput("cat %s/etc/hamonikr/info | grep GRUB_TITLE" % mount_point).replace('GRUB_TITLE', '').replace('=', '').replace('"', '').strip()
