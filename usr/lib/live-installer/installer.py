...
class InstallerEngine:
    ...
    def finish_installation(self):
        ...
        if os.path.exists("/target/usr/lib/hamonikr/hamonikr-system/hamonikr-adjust.py"):
            self.do_run_in_chroot("/usr/lib/hamonikr/hamonikr-system/hamonikr-adjust.py")
        ...
        self.do_run_in_chroot("/etc/hamonikr/adjustments/10-adjust-grub-title.execute")
        kernelversion= subprocess.getoutput("uname -r")
    ...
    def do_check_grub(self, our_total, our_current):
        ...
        if os.path.exists("/target/boot/grub/grub.cfg"):
            ...
            for line in grubfh:
                line = line.rstrip("\r\n")
                if ("menuentry" in line and ("class hamonikr" in line or "class linuxmint" in line or "Linux Mint" in line or "LMDE" in line)):
                    found_entry = True
